...
class GoogleSheetsConnector:
    def __init__(self, spreadsheet_id: str, credentials_path: str):
        ...
        self.sheets = {
            "referentiel": "REFERENTIEL", "config_fixe": "CONFIG_FIXE",
            "dico": "DICO_MATCHING", "transactions": "TRANSACTIONS",
            "epargne": "EPARGNE_HISTO", "a_classifier": "A_CLASSIFIER"
        }
        self._ws_cache: Dict[str, Any] = {}   # cache worksheet objects → 0 read API par appel répété
        self.gc = self._authenticate()
        if self.gc:
            try:
                self.sh = self.gc.open_by_key(spreadsheet_id)
                logger.info(f"✅ Connexion établie au classeur : '{self.sh.title}'")
            except Exception as e:
                logger.error(f"❌ Impossible d'ouvrir le classeur. Erreur: {e}")
                self.sh = None
        else:
            self.sh = None
    ...
    def load_sheet(self, sheet_key: str) -> pd.DataFrame:
        ...
        sheet_name = self.sheets.get(sheet_key, sheet_key)
        try:
            worksheet = self.sh.worksheet(sheet_name)
            return pd.DataFrame(worksheet.get_all_records())
        except Exception as e:
            logger.error(f"❌ Erreur lors du chargement de '{sheet_name}': {e}")
            return pd.DataFrame()

    def get_sheet(self, sheet_key: str) -> Optional[gspread.Worksheet]:
        if not self.sh: return None
        sheet_name = self.sheets.get(sheet_key, sheet_key)
        if sheet_name not in self._ws_cache:
            try:
                self._ws_cache[sheet_name] = self.sh.worksheet(sheet_name)
            except gspread.exceptions.WorksheetNotFound:
                logger.error(f"⚠️ La feuille '{sheet_name}' est introuvable.")
                return None
        return self._ws_cache[sheet_name]

    def ecrire_ligne(self, sheet_key: str, ligne: List[Any]):
...
class Trieur:
    ...
    def flush_a_classifier(self):
        ...
        try:
            ...
            if modifiees:
                cells = [gspread.Cell(row=e["ligne_index"], col=4, value=e["nb_occurrences"]) for e in modifiees]
    ...
    def _maj_epargne_histo(self, date_valeur: date, montant_abs: float):
        ...
        try:
            ...
            if ligne_trouvee:
                ...
                # Écriture atomique des deux cellules en un seul appel API
                ws.update_cells([
                    gspread.Cell(num_ligne, idx_reel + 1, nouveau_reel),
                    gspread.Cell(num_ligne, idx_evol + 1, evolution),
                ])
                logger.info(
